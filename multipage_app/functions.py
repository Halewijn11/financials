...
def set_theme(settings):
    src_filepath = f"./{settings['config_repo_path']}/{settings['theme']}.toml"
    dst_filepath = f"./{settings['streamlit_repo_path']}config.toml"
    shutil.copy(src_filepath,dst_filepath)

def treemap_df(core_transaction_df, settings):
    summary_df = core_transaction_df.groupby(['tag', 'main_tag'], as_index = False)['inverted_amount'].sum()
    summary_df['color'] = settings['background_color']  # add a constant color column

    fig = px.treemap(data_frame=summary_df,
            path=['main_tag', 'tag'],
            values = 'inverted_amount',
            maxdepth=settings['maxdepth'],
            color = 'color',
            color_discrete_sequence=[settings['background_color']]
            )
